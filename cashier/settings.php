<?php 
session_start();
if (empty($_SESSION['cashier']) OR empty($_SESSION['type'])) {
...
<head>
	<meta charset="UTF-8">
	<title>Cashier Account Settings Dashboard - HMS</title>
		...
		<div class="right"><br><br>
			...
			if (isset($btn) && !empty($fname)&& !empty($sname)&& !empty($password)&& !empty($password2)) {
				if ($password != $password2) {
					echo "<br><b style='color:red;font-size:14px;font-family:Arial;'>Passwords Must Match!</b>";
				}
				else{
					require "../includes/cashier.php";
					settings();
				}
